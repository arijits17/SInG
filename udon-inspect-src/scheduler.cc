...
max_errors(1000000), num_of_errors_detected(0), num_of_transitions(0), num_of_states(
	...
	Scheduler::~Scheduler() {
		printf("------- total writes (includes local) : %d\n", global_write_count);
...
void Scheduler::exec_transition(InspectEvent & event) {
	...
	if(event.type == OBJ_WRITE){
		global_write_count++;
		printf("I'm a write object from Scheduler\n");
	}

	if(event.type == OBJ_READ){
		printf("I'm a read object from scheduler\n");
		if (global_write_count>2){
			//need to remove this trace
			remove_this_trace = true;
...
void Scheduler::monitor_first_run() {
	...
	cout << " === RUN " << run_counter << " ===\n";
	cout << "  ==== global_write_count " << global_write_count << "==== \n";
	global_write_count = 0;
	this->exec_test_target(setting.target.c_str());
